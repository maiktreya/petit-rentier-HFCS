...
for (varname in var_code) { # nolint
    ...
    for (wave in path_stringB) {
        ...
        for (n in seq_along(country_code)) {
            ...
            for (m in 1:n_imps) {
                ...
                transf[, capital := rental + financ + pvpens + Kgains][, iscapital := as.logical(capital)]
            ...
            for (i in 1:5) means[i] <- svymean(as.formula(paste0("~", varname)), subset(designs[[i]], quintile.gwealth == 2 & varname > 0), na.rm = TRUE)[1] %>% unname()
    ...
    fwrite(mean_of_years, paste0("prod/survey_methods/out/", varname, "_wealthy.csv"))
