...
# Purpose: Only describe how including K-gains in CI affects
#          petit-rentier prevalence at chosen thresholds.
# Outputs: One compact CSV with weighted prevalences by spec.
############################################################
...
specs <- list(
    ...
    K_trim99    = function(d, t) make_rent_dummy(d, t, include_K = TRUE, trim_top = trim_top)
)
...
results <- rbindlist(lapply(thresholds, function(tau) {
    res_tau <- rbindlist(lapply(names(specs), function(sp) {
        ...
        # by hasKgains label
        by_k <- dt[,
            .(prev = wmean(rents_dummy, weights), N = .N),
            by = hasKgains_lbl
        ]
