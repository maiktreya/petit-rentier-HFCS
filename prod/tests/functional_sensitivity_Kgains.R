...
Quick pooled run: pooled fixed effects + pooled AMEs + RE/ICC snapshot.

Diagnostics pack:

Baseline vs K-inclusive vs K-trim99 (P99) models.

Δ-AME (%) table for headline covariates.

Mundlak (within vs contextual) for hasKgains.

RE/ICC comparison + country RE rank ρ.

Threshold sweep AMEs {5,10,15,20,30,40}%.

Leave-one-out (country and wave) influence with max % change.
...
If you want small-sample df in pooling, you can swap the FE pooling for miceadds::micombine() (kept simple here because HFCS N is large).

To decompose shifts due to rescaling vs confounding (KHB), we can bolt on a khb section later.

For control-function instrumentation at the country×wave level, you’ll need a macro table; happy to add a bolt-on once you give me variable names.


Want me to pre-wire a ggplot template that reads the CSVs and makes the 3–4 key figures (Δ-AME bars, RE BLUP scatter, threshold-sweep lines, LOO influence ridge)?
