...
for (i in 1:5) {
    ...
    model[[i]] <- glmer(
        rentsbi ~ as.factor(wave) + hsize + head_gendr + age + edu_ref + quantile.gwealth + quantile.gincome + class +
...
fwrite(combined_results, "output/MODELS/MICRO/ren-fin-pen/w-complete-fast-fact.csv")
