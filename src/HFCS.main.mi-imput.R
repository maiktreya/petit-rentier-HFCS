...
for (j in 1:5) final_dt_h[[j]] <- fread(paste0(path_string, "H", j, ".csv"))[, imp := j] %>% data.frame()
# JOINT PERSONAL MATRIX PRE SUMMING IMPUTATIONS (YEAR-WAVE)
for (k in 1:5) final_dt_p[[k]] <- fread(paste0(path_string, "P", k, ".csv"))[, imp := k] %>% data.frame()
...
hfcs <- svrepdesign(
    repweights = repweg,
    weights = ~HW0010,
    data = imputationList(imp_design),
    scale = 1,
