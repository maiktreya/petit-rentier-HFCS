...
outcome <- fread("output/MEANS/ren-fin-pro-pens/rentsbi.csv", header = TRUE) %>%
    unlist() %>%
    as.vector()
outcome2 <- fread("output/MEANS/ren-fin-pro-pens/rentsbi2.csv", header = TRUE) %>%
    unlist() %>%
    as.vector()
outcome5 <- fread("output/MEANS/ren-fin-pro-pens/rentsbi5.csv", header = TRUE) %>%
    unlist() %>%
    as.vector()
