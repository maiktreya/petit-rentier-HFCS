...
for (i in seq_along(sel_year)) {
        ...
        up_bound <- svyquantile(~actreales, sv_eff, quantiles = c(0.99))[1]$actreales[, "quantile"]
        lo_bound <- svyquantile(~actreales, sv_eff, quantiles = c(0.01))[1]$actreales[, "quantile"]
...
######## plot
jpeg(file = "output/rif/img/gini_lorentz._rstate.jpeg")
svylorenz(~actreales, convey_prep(sv_eff), na.rm = T)
dev.off()

jpeg(file = "output/rif/img/cdf_rstate.jpeg")
svycdf(~actreales, subset(sv_eff, actreales < up_bound & actreales > lo_bound & class == "capitalist"), na.rm = T)[[1]] %>%
        plot(col = "cyan", main = "Group comparison of wealth quantiles 2020")
svycdf(~actreales, subset(sv_eff, actreales < up_bound & actreales > lo_bound & class == "worker"), na.rm = T)[[1]] %>%
        lines(col = "blue")
svycdf(~actreales, subset(sv_eff, actreales < up_bound & actreales > lo_bound), na.rm = T)[[1]]  %>%
        lines(col = "red")
legend("bottomright", legend = c("Employers", "Workforce", "Total Pop."), col = c("cyan", "blue", "red"), lty = 1)
dev.off()

jpeg(file = "output/rif/img/histogram_rstate.jpeg")
...
jpeg(file = "output/rif/img/emp_histogram_rstate.jpeg")
par(mfrow = c(1, 2))
